use cosmwasm_std::{
    coin, from_binary, to_binary, Addr, AllBalanceResponse, BankQuery, CosmosMsg, Empty, IbcMsg,
...
fn simple_transfer() -> anyhow::Result<()> {
    ...
    let balances = app2
        .raw_query(
            to_binary(&QueryRequest::<Empty>::Bank(BankQuery::AllBalances {
        ...
        .into_result()?
        .unwrap();
    let balances: AllBalanceResponse = from_binary(&balances)?;
