use cosmwasm_std::{
    from_binary, to_binary, ChannelResponse, Empty, IbcChannel, IbcEndpoint, IbcOrder, IbcQuery,
...
fn channel_creation() -> anyhow::Result<()> {
    ...
    let channel_query = app1
        .raw_query(
            to_binary(&QueryRequest::<Empty>::Ibc(IbcQuery::Channel {
    ...
    let channel: ChannelResponse = from_binary(&channel_query)?;
